...
function convertTemperature(temp, scale){
    let convertCelcius = temp * 9/5 + 32;
    let convertFahrenheit = (temp - 32) * 5/9;
    if(scale === 'C'){
        return `${convertCelcius} (Fahrenheit)`;
    } else if (scale === 'F'){
        return `${convertFahrenheit} (Celcius)`;
    }
}
...
function createUsername(firstName, lastName){
    ...
    let firstPart = `${splitFirst[0]}${splitFirst[1]}${splitFirst[2]}`;
    let splitLast = lastName.split('');
    let secondPart = `${splitLast[0]}${splitLast[1]}${splitLast[2]}`;
    let count = firstName.length + lastName.length;
    return `${firstPart}${secondPart}${count}`;
}
